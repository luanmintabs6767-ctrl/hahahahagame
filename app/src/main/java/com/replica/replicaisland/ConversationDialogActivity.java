...
public class ConversationDialogActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (levelRow != -1 && levelIndex != -1 && index != -1) {
            // 1. 안전하게 레벨 트리 및 다이얼로그 리소스가 존재하는지 먼저 확인 처리
            LevelTree.Level level = LevelTree.get(levelRow, levelIndex);
            if (level != null && level.dialogResources != null) {
                if (character == 1 && level.dialogResources.character1Conversations != null && index < level.dialogResources.character1Conversations.size()) {
                    mConversation = level.dialogResources.character1Conversations.get(index);
                } else if (character != 1 && level.dialogResources.character2Conversations != null && index < level.dialogResources.character2Conversations.size()) {
                    mConversation = level.dialogResources.character2Conversations.get(index);
                }
            }

            // 2. ◀ [핵심 방어 코드] 만약 조건에 맞는 대화 데이터를 결국 못 찾았다면(null 이라면)
            if (mConversation == null) {
                android.util.Log.e("ReplicaIsland", "대화 데이터를 찾을 수 없어 대화창을 안전하게 종료합니다.");
                finish(); // 튕기지 않고 화면만 닫음
                return;
            }

            // 3. 데이터가 확실히 있을 때만 뷰 초기화 진행
