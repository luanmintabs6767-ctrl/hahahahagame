...
public class KeyboardConfigDialogPreference extends DialogPreference implements OnKeyListener {
    ...
    private View getConfigViewById(int id) {
        ...
        // switch-case를 if-else chain으로 변경하여
        // public static final int가 아닌 변수형 ID도 처리할 수 있도록 수정
        if (id == R.id.key_left) {
            config = mLeftBorder;
        } else if (id == R.id.key_right) {
            config = mRightBorder;
        } else if (id == R.id.key_jump) {
            config = mJumpBorder;
        } else if (id == R.id.key_attack) {
    ...
    public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
        ...
        if (mListeningId != 0) {
            ...
            // mListeningId를 기반으로 한 switch-case 문을 if-else 구조로 변경
            if (mListeningId == R.id.key_left) {
                mLeftText.setText(getKeyLabel(keyCode));
                mLeftKeyCode = keyCode;
            } else if (mListeningId == R.id.key_right) {
                mRightText.setText(getKeyLabel(keyCode));
                mRightKeyCode = keyCode;
            } else if (mListeningId == R.id.key_jump) {
                mJumpText.setText(getKeyLabel(keyCode));
                mJumpKeyCode = keyCode;
            } else if (mListeningId == mAttackText.getContext().getResources().getIdentifier("key_attack", "id", mAttackText.getContext().getPackageName()) || mListeningId == R.id.key_attack) {
                // 안정성을 위해 기존 형식을 유지한 비교문
