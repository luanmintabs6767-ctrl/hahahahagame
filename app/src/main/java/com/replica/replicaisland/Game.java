...
public class Game extends AllocationGuard {
    ...
    public void bootstrap(Context context, int viewWidth, int viewHeight, int gameWidth, int gameHeight, int difficulty) {
        if (!mBootstrapComplete) {
            ...
            // 💥 [수정] .bin 대신 새로 마련한 collision.json 스트림 주입
            InputStream collisionStream = context.getResources().openRawResource(R.raw.collision);
            collision.loadCollisionTiles(collisionStream);
    ...
    protected synchronized void goToLevel(LevelTree.Level level) {
        ...
        // 💥 [수정] JSON 파일로 변경된 구조에 맞추어 level_tree에 등록된 JSON 파일을 스트림으로 오픈 후 주입
        InputStream levelStream = params.context.getResources().openRawResource(level.resource);
        BaseObject.sSystemRegistry.levelSystem.loadLevel(level, levelStream, mGameRoot);
    ...
    public void setKeyConfig(int leftKey, int rightKey, int jumpKey, int attackKey) {
        BaseObject.sSystemRegistry.inputGameInterface.setKeys(leftKey, rightKey, jumpKey, attackKey, android.view.KeyEvent.KEYCODE_C);
    }
