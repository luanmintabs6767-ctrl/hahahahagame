...
public class PlayerComponent extends GameComponent {
    ...
    private static final float DASH_VELOCITY = 800.0f;
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        // Level 14 ease-of-use boost
        LevelSystem levelSystem = sSystemRegistry.levelSystem;
        if (levelSystem != null && levelSystem.getCurrentLevel() != null) {
            String levelName = levelSystem.getCurrentLevel().name;
            if (levelName != null && (levelName.contains("#014") || levelName.contains("#006"))) {
                if (parentObject.life < 5) {
                    parentObject.life = 5;
                }
                mFuelAirRefillSpeed = 0.5f; // Faster refill
            }
        }
     ...
     // Watch for hit reactions or death interrupting the state machine.
        ...
        switch(mState) {
            ...
            case STOMP:
                ...
                break;
            case DASH:
                stateDash(gameTime, timeDelta, parentObject);
                break;
            case HIT_REACT:
    ...
    protected void stateMove(float time, float timeDelta, GameObject parentObject) {
        if (!mGhostActive) {
            ...
            if (attackButton.getPressed() && jumpButton.getPressed() && dpad.getX() != 0.0f
                    && !input.isJumpAndAttackSameTouch()) {
                gotoDash(parentObject, dpad.getX());
            } else if (attackButton.getTriggered(time) && !mTouchingGround) {
    ...
    protected void gotoDash(GameObject parentObject, float direction) {
        parentObject.setCurrentAction(GameObject.ActionType.ATTACK);
        mState = State.DASH;
        mTimer = -1.0f;
        mDashDirection = direction;
        parentObject.getImpulse().zero();
        parentObject.getVelocity().set(0.0f, 0.0f);
    }
    
    protected void stateDash(float time, float timeDelta, GameObject parentObject) {
        if (mTimer < 0.0f) {
            mTimer = time;
        }
        
        final float dashDir = Utils.sign(mDashDirection);
        parentObject.getVelocity().set(dashDir * DASH_VELOCITY, 0.0f);
        parentObject.facingDirection.x = dashDir;
        
        // Manual collision check for breakable blocks/enemies in front during dash
        GameObjectManager manager = sSystemRegistry.gameObjectManager;
        if (manager != null) {
            FixedSizeArray<BaseObject> objects = manager.getObjects();
            if (objects != null) {
                final int count = objects.getCount();
                final float playerX = parentObject.getPosition().x;
                final float playerY = parentObject.getPosition().y;
                
                for (int i = 0; i < count; i++) {
                    GameObject target = (GameObject) objects.get(i);
                    if (target != parentObject && target.life > 0 && target.team == GameObject.Team.ENEMY) {
                        float dx = target.getPosition().x - playerX;
                        float dy = target.getPosition().y - playerY;
                        
                        boolean inFront = (dashDir > 0 && dx > 0 && dx < 80.0f) 
                                || (dashDir < 0 && dx < 0 && dx > -80.0f);
                        
                        if (inFront && Math.abs(dy) < 64.0f) {
                            HitReactionComponent hitReact = target.findByClass(HitReactionComponent.class);
                            if (hitReact != null) {
                                hitReact.receivedHit(target, parentObject, CollisionParameters.HitType.HIT);
                            }
                        }
                    }
                }
